...
export default function FounderImageCarousel({ data }) {
  ...
  return (
    ...
    <div
      id="divBtn"
      className="mx-auto relative imgCarousel overflow-hidden rounded-3xl"
    >
      <div className=" rounded-3xl h-full">
        <img
          src={data?.acf?.image}
          alt={data?.acf?.name}
          className="mx-auto w-[95%] h-[50%] sm:w-[90%] sm:h-full object-cover rounded-3xl"
        />
      </div>
      <div className="hidden lg:block absolute bottom-[22%] w-full imgText z-50">
        <div className="flex flex-col items-center justify-center leading-[1]">
          <p className="text-white text-[32px] 2xl:text-[50px] font-thin popinsFont"></p>
          <div
            id="nameText"
            className="flex items-center justify-center space-x-3"
          >
      ...
      <div className="absolute block w-full bottom-[4%] md:top-[88%] lg:hidden">
        <div className="mx-auto w-[170px] h-[30px] bg-white  cursor-pointer">
          <div className="flex items-center justify-center w-full h-full">
            <p
              className="popinsFont font-semibold text-[14px] text-black mb-0"
              onClick={() => {
      ...
      <div
        className="hidden lg:block  w-[90%] left-[5%] h-full absolute bgBlur"
        style={{
          background: `linear-gradient(0deg,${data.acf.gradient_color} 49%, ${data.acf.background_color}75%)`,
        }}
      ></div>
      ...
      <div className="btnInterview flex justify-center w-full">
        <div className="hidden lg:block w-[200px] h-[60px] bg-white  cursor-pointer z-50">
          <div id="seeF" className="flex  justify-center w-full h-full">
            <p
              className="popinsFont font-semibold text-[20px] text-black mb-0 pt-[7%]"
              onClick={() => {
                handleSeeFull(data);
              }}
            >
