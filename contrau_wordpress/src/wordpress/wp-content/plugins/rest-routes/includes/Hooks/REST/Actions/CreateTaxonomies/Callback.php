...
use RestRoutes\Hooks\REST\Actions\CreateTaxonomies\TaxonomyCreation;
...
class Callback
{
    ...
    public function getCallback(\WP_REST_Request $data)
    {
        ...
        do_action('rest_routes_before_create_taxonomies_callback', $data, $endpoint);
        ...
        foreach ($endpoint->getCreateTaxonomies() as $createTaxonomy) {
            ...
            if (! $term_id) {
                wp_send_json_error(['error' => 'Taxonomy term could not be created']);
            }

            $shouldRunCustomFields = apply_filters('rest_routes_should_process_cf_on_taxonomy_creation', true, $endpoint, $data, $term_id);
        ...
        do_action('rest_routes_after_create_taxonomies_callback', $data, $endpoint, $term_id);

        $result = [ 'message' => 'Taxonomy created', 'id' => $term_id ];

        wp_send_json_success(apply_filters('rest_routes_create_taxonomy_result', $result, $endpoint, $data, $term_id));
    }
}
