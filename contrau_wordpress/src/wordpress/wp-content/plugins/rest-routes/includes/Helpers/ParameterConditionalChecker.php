...
class ParameterConditionalChecker
{
    ...
    {
        $conditionalComparison = FieldValueParser::getFieldValue($outputField['fields'], 'compare');
        $conditionalValue      = FieldValueParser::getFieldValue($outputField['fields'], 'conditionalValue');

        if (!$conditionalValue || !$conditionalComparison) {
            return true;
        }

        $parameter = $request->get_param(FieldValueParser::getFieldValue($outputField['fields'], 'parameters'));
